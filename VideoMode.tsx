...
const VideoMode: React.FC = () => {
  ...
  // God Mode Inputs
  const [model, setModel] = useState('veo3.1-fast-components');
  ...
  const pollVideoStatus = async (taskId: string) => {
    ...
    const poll = async () => {
      try {
        const response = await fetch(`${pollingPath}?task_id=${taskId}`, {
          headers: {
        ...
        // 如果状态是等待中、运行中或处理中，继续轮询
        if (status === "pending" || status === "running" || status === "processing") {
          console.log(`视频生成中... (状态: ${status}, 第 ${attempts + 1}/${maxAttempts} 次轮询)`);
        ...
        // 未知状态
          ...
          console.warn(`未知的状态: ${status}`, data);
          // 如果状态不明确但没有错误，继续轮询
          if (attempts < maxAttempts) {
            attempts++;
            setTimeout(poll, pollInterval);
          } else {
            alert("视频生成状态异常，轮询已超时");
            setLoading(false);
          }
        }
      } catch (error) {
        ...
        if (attempts < maxAttempts) {
          attempts++;
          console.log(`轮询重试中... (第 ${attempts}/${maxAttempts} 次)`);
          setTimeout(poll, pollInterval);
        } else {
          alert("轮询失败且已超时，请稍后手动查询");
          setLoading(false);
        }
      }
    };
